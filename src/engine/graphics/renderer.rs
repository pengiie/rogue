...
use super::{
    backend::{GraphicsBackendFrameGraphExecutor, Image},
    device::DeviceResource,
    frame_graph::{FGComputeInfo, FrameGraph},
};
...
impl Renderer {
    ...
    fn construct_fallback_frame_graph() -> FrameGraph {
        let mut builder = FrameGraph::builder();
        ...
        builder.create_pass(
            ...
            |recorder, ctx| {
                let swapchain_image = ctx.get_image(Self::GRAPH_SWAPCHAIN_NAME);
                recorder.clear_color(swapchain_image, Color::new(1.0, 0.0, 0.0));
            },
        );
    ...
    pub fn begin_frame(mut renderer: ResMut<Renderer>, device: ResMut<DeviceResource>) {
        let renderer: &mut Renderer = &mut renderer;

        let frame_graph = renderer
            .frame_graph
            .take()
            .unwrap_or_else(|| Self::construct_fallback_frame_graph());
