...
impl<'a> ShaderWriter<'a> {
    ...
    pub fn write_binding_array<T: 'static>(
        ...
        binding_resources: &[Option<ResourceId<T>>],
    ) {
            ...
            ShaderBindingType::StorageBufferArray { size } => {
                ...
                Binding::StorageBufferArray {
                    buffers: binding_resources
                        .into_iter()
                        .map(|id| id.map(|id| ResourceId::new(id.id())))
                        .collect::<Vec<_>>(),
                }
            }
            ShaderBindingType::SampledImageArray { size } => {
                assert_eq!(std::any::TypeId::of::<T>(), std::any::TypeId::of::<Image>());
                Binding::SampledImageArray {
                    images: binding_resources
                        .into_iter()
                        .map(|id| id.map(|id| ResourceId::new(id.id())))
                        .collect::<Vec<_>>(),
                }
            }
            ShaderBindingType::SamplerArray { size } => {
                assert_eq!(
                    std::any::TypeId::of::<T>(),
                    std::any::TypeId::of::<Sampler>()
                );
                Binding::SamplerArray {
                    samplers: binding_resources
                        .into_iter()
                        .map(|id| id.map(|id| ResourceId::new(id.id())))
                        .collect::<Vec<_>>(),
                }
            }
            ty => panic!("Type {:?} not supported for arrays", ty),
        };
...
pub enum Binding {
    ...
    StorageBufferArray {
        buffers: Vec<Option<ResourceId<Buffer>>>,
    },
    SampledImageArray {
        images: Vec<Option<ResourceId<Image>>>,
    },
    SamplerArray {
        samplers: Vec<Option<ResourceId<Sampler>>>,
    },
}
