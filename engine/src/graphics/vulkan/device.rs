...
impl VulkanDevice {
    pub fn init(
        ...
        let instance = {
            ...
            if enable_debug {
                let validation_layer_cstring =
                    std::ffi::CString::new(VALIDATION_LAYER_NAME).unwrap();
            ...
            let validation_layer_cstr =
                std::ffi::CString::new("VK_LAYER_KHRONOS_validation").unwrap();
            let print_f_cstr = std::ffi::CString::new("printf_enable").unwrap();
            let print_f_cstr_name =
                std::ffi::CString::new("VK_VALIDATION_FEATURE_ENABLE_DEBUG_PRINTF_EXT").unwrap();
            ...
            let mut layer_settings_info =
                ash::vk::LayerSettingsCreateInfoEXT::default().settings(&layer_settings);
        ...
        let physical_device = {
            let mut devices = unsafe { instance.enumerate_physical_devices() }?
                ...
                .filter_map(|physical_device| {
                    ...
                    let mut properties2 =
                        ash::vk::PhysicalDeviceProperties2::default().push_next(&mut maintenance3);
                    unsafe {
            ...
            for (i, score) in &scored_devices {
                let name = devices[*i]
                    ...
                    .map_or("Unknown device".to_owned(), |c_name| {
        ...
        let device = {
            ...
            feature_dynamic_rendering_local_read.p_next =
                std::ptr::from_mut(&mut feature_descriptor_indexing) as *mut std::ffi::c_void;
...
impl GraphicsBackendDevice for VulkanDevice {
    ...
    fn device_info(&self) -> GfxDeviceInfo {
        ...
        GfxDeviceInfo {
            ...
            max_storage_buffer_array_binding_count: dev
                .properties
                .limits
...
impl VulkanAllocator {
    ...
    fn find_or_create_shared_memory(
        ...
        return self.create_shared_memory(
            allocation_size,
...
impl VulkanResourceManager {
    ...
    fn retire_resources(&self) {
        ...
        for set_group in descriptor_set_groups.values_mut() {
            set_group.binding_set_maps[self.ctx.curr_cpu_frame_index() as usize]
                .uniform_buffer_counter = 0;
        }
    ...
    fn create_descriptor_set_layout(
        ...
        let (mut layout_bindings, mut layout_binding_flags): (
            Vec<ash::vk::DescriptorSetLayoutBinding>,
            Vec<ash::vk::DescriptorBindingFlags>,
        ) = set_binding
            .bindings
            .iter()
            .filter_map(|(binding_name, (binding, _is_used))| 'map: {
                let vk_binding_type = match binding.binding_slot_type() {
                    Some(ty) => match ty {
                        ShaderBindingType::Sampler | ShaderBindingType::SamplerArray { .. } => {
                ...
                let descriptor_count = match binding.binding_slot_type().unwrap() {
                    ...
                    ShaderBindingType::SampledImageArray { size } => size,
                    ShaderBindingType::SamplerArray { size } => size,
                    _ => 1,
                };
                ...
                // Make this configurable when writing the binding at some point, or do bindless
                // descriptors, idk.
                let binding_flags = if descriptor_count > 1 {
                    ash::vk::DescriptorBindingFlags::PARTIALLY_BOUND
                } else {
                    ash::vk::DescriptorBindingFlags::empty()
                };

                Some((vk_binding, binding_flags))
            })
            .unzip();
        ...
        if let Some(global_uniform_binding_index) = set_binding.global_uniform_binding_index {
            ...
            layout_binding_flags.push(ash::vk::DescriptorBindingFlags::empty());
        }

        let mut binding_flag_create_info =
                ...
                .or_insert_with_key(|bindings| {
                    ...
                    for (binding_idx, binding) in bindings.bindings.iter() {
                        match binding {
                            ...
                            Binding::SampledImageArray { images } => {
                                for (i, image) in images.iter().enumerate() {
                                    let Some(image) = image else {
                                        continue;
                                    };
                                    let image_info = self.get_image_info(image);
                                    vk_image_infos.push(ash::vk::DescriptorImageInfo::default()
                                        .image_view(image_info.view.unwrap()));
                                    let write = ash::vk::WriteDescriptorSet::default()
                                        .dst_set(new_set)
                                        .dst_binding(*binding_idx)
                                        .dst_array_element(i as u32)
                                    ...
                                    let set_index = VulkanSetWriteIndex::ImageInfo(
                                        vk_image_infos.len() - 1,
                                    );
                                    vk_descriptor_set_writes.push((write, set_index));
                                }
                        ...
                        match binding {
                            ...
                            Binding::StorageBufferArray { .. } |
                                Binding::SampledImageArray { .. } |
                                Binding::SamplerArray {..} => unreachable!(),
                        }
    ...
    fn write_image(&self, allocator: &mut VulkanAllocator, info: GfxImageWrite) {
        ...
        assert_eq!(
            ...
            "Expected image data of {} bytes but we got {} bytes of data were provided for image of size {}x{} and format {:?}",
            write_len,
            info.data.len(),
            info.extent.x,
            info.extent.y,
            image_info.info.format
        );
    ...
    fn get_image_info(&self, image: &ResourceId<Image>) -> VulkanImage {
        let owned_images = self.owned_images.read();
        let Some(image_info) = owned_images.get(image) else {
            panic!("Tried to get buffer info of an image that doesn't exist.");
        };
        image_info.clone()
    }
