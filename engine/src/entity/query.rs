...
use crate::entity::{
    ...
    component::{ComponentRef, ComponentRefMut, ComponentTypeBorrow},
...
pub trait Query {
    ...
    fn collect_required_type_ids() -> Vec<TypeId>;
...
pub trait QueryItem {
    ...
    fn required_type_id() -> Option<TypeId>;
    fn borrow(archetype: &ComponentArchetype) -> Option<&Cell<ComponentTypeBorrow>>;
    fn fetch(archetype: &ComponentArchetype, index: usize) -> Self::Item<'_>;
}
...
impl<T: 'static> QueryItem for &'_ T {
    ...
    fn borrow(archetype: &ComponentArchetype) -> Option<&Cell<ComponentTypeBorrow>> {
        Some(archetype.borrow_type(&std::any::TypeId::of::<T>()))
    }
...
impl<T: 'static> QueryItem for &'_ mut T {
    ...
    fn borrow(archetype: &ComponentArchetype) -> Option<&Cell<ComponentTypeBorrow>> {
        Some(archetype.borrow_type_mut(&std::any::TypeId::of::<T>()))
    }
...
impl<T: 'static> QueryItem for Option<&'_ T> {
    ...
    fn borrow(archetype: &ComponentArchetype) -> Option<&Cell<ComponentTypeBorrow>> {
        archetype.try_borrow_type(&std::any::TypeId::of::<T>())
    }

    fn fetch(archetype: &ComponentArchetype, index: usize) -> Self::Item<'_> {
        return archetype.try_get::<T>(&TypeInfo::new::<T>(), index);
    }
}

impl<T: 'static> QueryItem for Option<&'_ mut T> {
    ...
    fn borrow(archetype: &ComponentArchetype) -> Option<&Cell<ComponentTypeBorrow>> {
        archetype.try_borrow_type_mut(&std::any::TypeId::of::<T>())
    }
    ...
    fn fetch(archetype: &ComponentArchetype, index: usize) -> Self::Item<'_> {
        ...
        return unsafe {
            archetype
                .try_get_mut_unchecked::<T>(&TypeInfo::new::<T>(), index)
...
macro_rules! impl_query {
    ($($param:ident),+ , $($num:literal),*) => {
        impl<$($param: QueryItem),*> Query for ($($param,)*) {
            ...
            fn collect_required_type_ids() -> Vec<TypeId> {
                vec![ $(<$param as QueryItem>::required_type_id()),* ]
                    .into_iter()
                    .filter_map(|ty| ty)
                    .collect::<Vec<_>>()
            }
            ...
            fn collect_borrows(archetype: &ComponentArchetype) -> Vec<&Cell<ComponentTypeBorrow>> {
                vec![ $($param::borrow(archetype)),* ]
                    .into_iter()
                    .filter_map(|borrow| borrow)
                    .collect::<Vec<_>>()
            }
...
impl<T: QueryItem> Query for T {
    ...
    fn collect_required_type_ids() -> Vec<TypeId> {
        T::required_type_id().into_iter().collect::<Vec<_>>()
    }

    fn collect_borrows(archetype: &ComponentArchetype) -> Vec<&Cell<ComponentTypeBorrow>> {
        T::borrow(archetype).into_iter().collect()
    }
...
impl<'a, Q: Query> QueryBorrow<'a, Q> {
    pub fn new(ecs_world: &'a ECSWorld) -> Self {
        Self {
            ecs_world,
            with: Q::collect_required_type_ids()
                .into_iter()
                .collect::<HashSet<_>>(),
            without: HashSet::new(),
