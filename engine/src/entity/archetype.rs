...
impl ComponentArchetype {
    ...
    fn get_type_index(&self, type_id: &TypeId) -> usize {
        self.try_get_type_index(type_id).unwrap_or_else(|| {
            panic!(
                "Type id {:?} is not a part of this archetype. This archetype has types: {:?}",
                type_id,
                self.types
                    .iter()
                    .map(|t| format!("Type {}, id: {:?}", t.name(), t.type_id))
                    .collect::<Vec<_>>()
            )
        })
    }

    fn try_get_type_index(&self, type_id: &TypeId) -> Option<usize> {
        self.types
            .iter()
            .enumerate()
    ...
    fn try_get_type_data(&self, type_info: &TypeInfo) -> Option<&DynVec> {
        self.try_get_type_index(&type_info.type_id)
            .map(|i| &self.data[i])
    }
    ...
    fn try_get_type_data_mut(&mut self, type_info: &TypeInfo) -> Option<&mut DynVec> {
        self.try_get_type_index(&type_info.type_id)
    ...
    pub fn try_get<T: 'static>(&self, type_info: &TypeInfo, index: usize) -> Option<&T> {
        ...
        self.try_get_type_data(type_info)
    ...
    /// For unsafety please see `DynVec::get_mut_unchecked(usize)`.
    pub unsafe fn try_get_mut_unchecked<T: 'static>(
        &self,
        type_info: &TypeInfo,
        index: usize,
    ) -> Option<NonNull<T>> {
        ...
        return self
            .try_get_type_data(type_info)
            .map(|data| data.get_mut_unchecked(index));
    }
    ...
    pub fn try_borrow_type(&self, type_id: &TypeId) -> Option<&Cell<ComponentTypeBorrow>> {
        self.try_get_type_index(type_id)
            .map(|index| self.borrow_type_at_index(index))
    }

    pub fn borrow_type(&self, type_id: &TypeId) -> &Cell<ComponentTypeBorrow> {
        let index = self.get_type_index(type_id);
        return self.borrow_type_at_index(index);
    }

    fn borrow_type_at_index(&self, index: usize) -> &Cell<ComponentTypeBorrow> {
    ...
    /// Tries borrowing the component type exclusively if the type exists. This will
    /// still panic if the type is already borrowed.
    pub fn try_borrow_type_mut(&self, type_id: &TypeId) -> Option<&Cell<ComponentTypeBorrow>> {
        self.try_get_type_index(type_id)
            .map(|index| self.borrow_type_mut_at_index(index))
    }

    pub fn borrow_type_mut(&self, type_id: &TypeId) -> &Cell<ComponentTypeBorrow> {
        let index = self.get_type_index(type_id);
        return self.borrow_type_mut_at_index(index);
    }

    fn borrow_type_mut_at_index(&self, index: usize) -> &Cell<ComponentTypeBorrow> {
        let borrow = &self.borrows[index];
        assert!(
