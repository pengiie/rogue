...
impl<T> FreeListHandle<T> {
    ...
    pub const fn null() -> FreeListHandle<T> {
        Self::DANGLING
    }
...
impl<T> FreeList<T> {
    ...
    pub fn iter_all(&self) -> FreeListFullIterator<'_, T> {
        FreeListFullIterator {
            free_list: self,
            left: 0,
        }
    }
...
pub struct FreeListFullIterator<'a, T> {
    free_list: &'a FreeList<T>,
    left: usize,
}

impl<'a, T> Iterator for FreeListFullIterator<'a, T> {
