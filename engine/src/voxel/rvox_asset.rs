...
use crate::{
    ...
    material::{
        material_bank::MaterialId,
        model_material_map::{ModelMaterial, ModelMaterialMap},
    },
    voxel::{
...
impl AssetLoader for RVOXAsset {
        ...
        Self: Sized + std::any::Any,
    {
        ...
        cursor += 4;
        load_v1(&buf[cursor..])
            .map_err(|e| anyhow::anyhow!("Failed to load RVOX asset: {:?}", e).into())
    }
}
...
impl AssetSaver for RVOXAsset {
        ...
        Self: Sized,
    {
        ...
        let model_material_data = sft
            .material_map
            .model_materials
            .iter()
            .map(|m| m.material_id)
            .collect::<Vec<_>>();
        bytes.extend_from_slice(&(model_material_data.len() as u32).to_le_bytes());
        bytes.extend_from_slice(bytemuck::cast_slice(model_material_data.as_slice()));
...
fn load_v1(buf: &[u8]) -> anyhow::Result<RVOXAsset> {
    let mut cursor = 0;

    let length = u32::from_le_bytes(buf[cursor..(cursor + 4)].try_into().unwrap());
    cursor += 4;
    let node_data_len = u32::from_le_bytes(buf[cursor..(cursor + 4)].try_into().unwrap()) as usize;
    cursor += 4;
    ...
    .to_vec();
    cursor += attachment_data_len * Attachment::BMAT.byte_size() as usize;

    let model_material_data_len =
        u32::from_le_bytes(buf[cursor..cursor + 4].try_into().unwrap()) as usize;
    cursor += 4;
    let model_material_data = bytemuck::cast_slice::<u8, MaterialId>(
        &buf[cursor..(cursor + model_material_data_len * std::mem::size_of::<MaterialId>())],
    )
    .to_vec();
    let mut material_map = ModelMaterialMap {
        model_materials: model_material_data
            .into_iter()
            .enumerate()
            .map(|(i, material_id)| ModelMaterial {
                material_id,
                model_material_id: i as u32,
            })
            .collect(),
    };
