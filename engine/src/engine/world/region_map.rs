...
use crate::{
    ...
    engine::{
        ...
        event::Events,
        resource::ResMut,
        voxel::voxel::VoxelEditData,
        world::region::{ChunkPos, WorldRegion},
    },
};
    ...
    pub fn update_region_streaming(
        ...
        for (region_pos, asset_handle) in &mut region_map.loading_regions {
            if region_map.regions.contains_key(region_pos) {
                finished_loading.push(*region_pos);
                continue;
            }

            let Some(ref mut asset_handle) = asset_handle else {
                *asset_handle = assets.load_asset::<WorldRegion>(AssetPath::new_game_assets_dir(
                    assets.project_assets_dir().unwrap(),
                    format!("region_{}_{}_{}", region_pos.x, region_pos.y, region_pos.z),
                ));
                continue;
            };

            let make_empty_region = false;
