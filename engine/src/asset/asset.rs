...
macro_rules! impl_asset_load_save_serde {
    ($name:ident) => {
        impl $crate::asset::asset::AssetLoader for $name {
            fn load(
                    ...
                    "json" | "rmat" => match data.read_contents() {
                        ...
                        Err(err) => match err.kind() {
                            std::io::ErrorKind::NotFound => {
                                Err($crate::asset::asset::AssetLoadError::NotFound { path: None })
                            }
                            _ => Err($crate::asset::asset::AssetLoadError::Other(
    ...
    pub fn as_relative_path(&self) -> PathBuf {
        ...
        while let Some(str) = strs.next() {
            if let Some(last_last_iter_str) = last_last_str {
                ...
                path = path.join(last_last_iter_str);
            }
